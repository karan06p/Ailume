...
export default function MobileSidebar() {
  ...
  return (
    ...
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger onClick={() => setOpen(true)} ><LuAlignJustify className="text-2xl"/> </SheetTrigger>
      <SheetContent>
        <SheetHeader>
        ...
        <div className="flex flex-col gap-5">
            {routes.map((item, index) => (
              ...
              >
                <Link href={item.route} className="py-2 px-2 rounded-full flex items-center justify-start font-mono w-full gap-4 text-xl">{item.icon} {item.title}</Link>
              </button>
            ))}
          </div>
        </div>
